...
import { CHECKPOINT_TYPE, MINUTES_CONVERSION, NODE_TYPES, PLATFORM_TYPES, RECOVERY_ENTITY_TYPES, REVERSE_ENTITY_TYPE, REVERSE_REPLICATION_TYPE, STATIC_KEYS, UI_WORKFLOW } from '../constants/InputConstants';
...
export function getNodePayload(user) {
  ...
  if (!shouldShowNodeReplicationPort(user)) {
    node.node.replicationCtrlPort = 5000; // For management node replication ctrl port should be 5000
  }
  if (node.node.nodeType === NODE_TYPES.DEDUPE_SERVER || node.node.nodeType === NODE_TYPES.PrepNode) {
