...
import { API_BULK_GENERATE, API_GET_BULK_PLANS, API_GET_CONFIG_TEMPLATE_BY_ID, API_GET_PLAN_DIFF, API_UPDATE_ISPLAYBOOK_DOWNLOAD_STATUS, API_UPLOAD_TEMPLATED, API_VALIDATE_TEMPLATE, CREATE_PLAN_FROM_PLAYBOOK, routeStart } from '../../constants/ApiConstants';
    ...
    return callAPI(CREATE_PLAN_FROM_PLAYBOOK.replace('<id>', id), obj).then((json) => {
      ...
      if (json.hasError) {
        ...
        if (status !== 'configPlanReconfigured' && typeof name !== 'undefined') {
          const result = `/${routeStart}playbooks/${json.name}`;
    ...
    callAPI(url).then((json) => {
      ...
      if (json.hasError) {
        ...
        if (typeof name !== 'undefined') {
          const result = `/${routeStart}playbooks/download/${json.name}`;
          const link = document.createElement('a');
          link.href = result;
          link.click();
        }
...
export function downloadPlaybooks(data, dispatch) {
  ...
  const obj = createPayload(API_TYPES.POST, payload);
  dispatch(showApplicationLoader('download_single_playbook', 'Downloading...'));
  callAPI(API_BULK_GENERATE, obj).then((json) => {
    dispatch(hideApplicationLoader('download_single_playbook'));
    if (json) {
      if (json.name && typeof json.name !== 'undefined') {
        ...
        const downloadURL = `${window.location.protocol}//${window.location.host}/${routeStart}playbooks/${json.name}`;
        const link = document.createElement('a');
...
export function downloadDateModifiedPlaybook(name) {
  const result = `/${routeStart}playbooks/download/${name}`;
  ...
  const link = document.createElement('a');
  nameSplitByDash.pop();
  link.download = `${nameSplitByDash.join('_')}-${resp}.xlsx`;
  link.href = result;
  link.click();
}
