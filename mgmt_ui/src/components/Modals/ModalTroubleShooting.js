...
import { API_FETCH_DR_PLAN_BY_ID, routeStart } from '../../constants/ApiConstants';
...
function ModalTroubleShooting(props) {
  ...
  useEffect(() => {
    ...
    callAPI(API_FETCH_DR_PLAN_BY_ID.replace('<id>', protectionPlanID)).then((json) => {
      ...
      const { hostname = '' } = node;
      setSourceNodePlanURL(`https://${hostname}:${node.managementPort}/${routeStart}protection/plan/details/${remoteProtectionPlanId}?reset=true`);
    },
    (err) => {
      if (isUnmounting) return;
