    ...
    if (platfromType === PLATFORM_TYPES.VMware) {
      return (
        <Container fluid className="padding-top-20">
          ...
          <DMTree dispatch={dispatch} search searchURL={`mgmt/api/v1/sites/${id}/vms`} user={user} selectedData={selectedVMs} showSelectedvmdata fieldKey="ui.site.vmware.selectedvms" field={field} selectedVMkey="ui.selectedvm.value" vmCss={vmCss} selectedVmCss={selectedVmCss} />
        </Container>
      );
    }
    return (
