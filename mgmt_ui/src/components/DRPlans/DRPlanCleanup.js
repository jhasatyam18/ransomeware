...
import { drPlanDetailsFetched, fetchDRPlanById } from '../../store/actions/DrPlanActions';
...
import { cleanupResources, cleanupResourcesFetched, cleanupToggleChildRow, fetchCleanupResources, handleCleanupTableSelection, handleSelectAllCleanupResources, onFilterTable, setCleanupData, updateSelectedCleanupResources } from '../../store/actions/cleanupActions';
...
function DRPlanCleanup({ drPlans, t, dispatch, user }) {
  ...
  return (
    <Container fluid>
      <Card>
        {Object.keys(protectionPlan).length > 0 ? (
          <CardBody>
            {planName ? (
              <DMBreadCrumb
                links={[
