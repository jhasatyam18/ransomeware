  ...
  const renderActions = () => {
    ...
    if (localVMIP === recoverySite.node.hostname) {
      if (checkpointType === RECOVERY_CHECKPOINT_TYPE.PRESERVED_CHECKPOINTS) {
        return <ActionButton label="Delete Checkpoint" onClick={() => dispatch(openDeleteCheckpointModal())} icon={faTrash} t={t} key="delet" isDisabled={selectedCheckpointsKeys.length === 0 || !hasRequestedPrivileges(user, ['checkpoint.delete'])} />;
      }
