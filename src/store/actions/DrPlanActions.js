import { REVERSE_CONFIG_STEP } from '../../constants/DrplanConstants';
...
export function setReverseConfig(protectionPlan) {
  return (dispatch) => {
    ...
    dispatch(valueChange(REVERSE_CONFIG_STEP.ID, protectionPlan.id));
    dispatch(valueChange(REVERSE_CONFIG_STEP.REMOTEPPLAN_ID, protectionPlan.remoteProtectionPlanId));
    dispatch(valueChange(REVERSE_CONFIG_STEP.NAME, protectionPlan.name));
    dispatch(valueChange(REVERSE_CONFIG_STEP.BOOTDELAY, protectionPlan.bootDelay));
    dispatch(valueChange(REVERSE_CONFIG_STEP.PRE_SCRIPT, protectionPlan.preScript));
    dispatch(valueChange(REVERSE_CONFIG_STEP.POST_SCRIPT, protectionPlan.postScript));
    dispatch(valueChange(REVERSE_CONFIG_STEP.PRE_INPUT, protectionPlan.preInputs));
    dispatch(valueChange(REVERSE_CONFIG_STEP.POST_INPUT, protectionPlan.postInputs));
    dispatch(valueChange(REVERSE_CONFIG_STEP.PROTECTED_SITE, protectionPlan.protectedSite.id));
    dispatch(valueChange(REVERSE_CONFIG_STEP.RECOVERY_SITE, protectionPlan.recoverySite.id));
    dispatch(valueChange(REVERSE_CONFIG_STEP.REPLICATION_INTERVAL, protectionPlan.replicationInterval));
    dispatch(valueChange(REVERSE_CONFIG_STEP.SCRIPT_TIMEOUT, protectionPlan.scriptTimeout));
    dispatch(valueChange(REVERSE_CONFIG_STEP.PROTECTED_ENTITIES_ID, protectedEntities.id));
    dispatch(valueChange(REVERSE_CONFIG_STEP.RECOVERY_ENTITIES_ID, recoveryEntities.id));
    dispatch(valueChange(REVERSE_CONFIG_STEP.STATUS, protectionPlan.status));
    dispatch(valueChange(REVERSE_CONFIG_STEP.PLAN_ID, protectionPlan.id));
    const time = protectionPlan.startTime * 1000;
    const d = new Date(time);
    dispatch(valueChange(REVERSE_CONFIG_STEP.START_TIME, d));
    dispatch(valueChange(REVERSE_CONFIG_STEP.IS_ENCRYPTION, protectionPlan.isEncryptionOnWire));
    dispatch(valueChange(REVERSE_CONFIG_STEP.IS_ENCRYPTION_REST, protectionPlan.isEncryptionOnRest));
    dispatch(valueChange(REVERSE_CONFIG_STEP.PLAN_COMPRESSION, protectionPlan.isCompression));
    dispatch(valueChange(REVERSE_CONFIG_STEP.IS_DEDUPE, protectionPlan.isDeDupe));
    dispatch(valueChange(REVERSE_CONFIG_STEP.ENABLE_REVERSE, protectionPlan.enableReverse));
    dispatch(valueChange(REVERSE_CONFIG_STEP.REPLICATION_POST_SCRIPT, protectionPlan.replPostScript));
    dispatch(valueChange(REVERSE_CONFIG_STEP.REPLICATION_PRE_SCRIPT, protectionPlan.replPreScript));
  };
}
