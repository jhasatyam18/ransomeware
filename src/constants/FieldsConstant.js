...
export const FIELDS = {
  ...
  'configureSite.platformDetails.region': {
    label: 'region', description: 'Cloud Site Region where Management Server is deployed and Protection needs to be done', type: FIELD_TYPE.SELECT, errorMessage: 'Region required', validate: (value, user) => isEmpty(value, user), shouldShow: (user) => !isPlatformTypeVMware(user), options: (user) => getRegionOptions(user), onChange: (user, dispatch) => updateAvailabilityZones(user, dispatch), fieldInfo: 'info.site.region',
  },
  ...
  'configureSite.platformDetails.secretKey': {
    label: 'secret.key', description: 'Secret Key for Cloud Site(AWS)', type: FIELD_TYPE.PASSWORD, validate: (value, user) => isEmpty(value, user), shouldShow: (user) => isPlatformTypeAWS(user) || isPlatformTypeAzure(user), fieldInfo: 'info.site.secrete.key', errorMessage: 'Secret key is required.',
  },
  ...
  'drplan.enableDifferentialReverse': { label: 'Differential Reverse Replication', description: 'Enable For Reverse', type: FIELD_TYPE.CHECKBOX, shouldShow: (user) => showDifferentialReverseCheckbox(user), defaultValue: false, fieldInfo: 'info.protectionplan.enable.reverse', onChange: ({ value }) => onDiffReverseChanges({ value }) },
  ...
  'recovery.installSystemAgent': { label: 'recovery.installSystemAgent', description: 'Install System Agents', placeHolderText: '', type: FIELD_TYPE.CHECKBOX, validate: null, errorMessage: '', shouldShow: (u) => showInstallSystemAgentOption(u), defaultValue: false, fieldInfo: 'info.recovery.system.agent' },
  ...
  'ui.installSystemAgent.warning': { type: FIELD_TYPE.STATICTEXT, shouldShow: (user) => showinstallCloudPkgWarningText(user), text: i18n.t('recover.cloud.agent.warning'), icon: 'fas fa-exclamation-triangle', textType: NOTE_TEXT.WARNING, className: 'card_node mb-2' },
  ...
  'drplan.removeCheckpoint': { label: 'Remove Associated Checkpoints', validate: ({ value, user }) => isEmpty({ value, user }), placeHolderText: 'Delete Recovery checkpoint of recovered vm/s', type: FIELD_TYPE.CHECKBOX, shouldShow: (user) => revShowRemoveCheckpointOption(user), errorMessage: 'Please provide Description', fieldInfo: 'Remove Recovery checkpoints for the recovered vm/s' },
  // idp
