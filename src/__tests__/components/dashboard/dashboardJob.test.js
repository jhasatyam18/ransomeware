...
describe('DashboardJob : Dashboard Job tests',()=>{
    ...
    it('should render without error',async()=>{
        ...
        let text= await screen.findByText("Jobs")
        expect(text).toBeInTheDocument()
    })
    it('should render data with response as started and failed',async()=>{
        renderWitRedux(<DashboardJob/>)
        let text= await screen.findByText("Jobs")
        let div= await screen.findByText("Recovery for prod-ubuntu-16-04 is started.")
        expect(text).toBeInTheDocument()
    ...
    it('should render data with response as completed and running',async()=>{
        ...
        renderWitRedux(<DashboardJob/>)
        let text= await screen.findByText("Jobs")
        let div= await screen.findByText("Recovery for prod-ubuntu-16-04 is completed.")
        expect(text).toBeInTheDocument()
        expect(div).toBeInTheDocument()
    })
    xit('should render data with replication respose as completed and running',async()=>{
        server.use(jobReplicationResp)
        renderWitRedux(<DashboardJob/>)
        let text= await screen.findByText("Jbs")
