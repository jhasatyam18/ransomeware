...
const DMCollapsibleRow = (props) => {
  const { user, dispatch, data = [], primaryKey, columns = [], index, isSelectable, onSelect, selectedData, toggleRow, tblName } = props;
  let cData = null;
  const onChange = (e) => {
  ...
  const onChildChange = (e, d) => {
    dispatch(onSelect(d, e.target.checked, primaryKey));
  };

  const getItemRenderer = (render, d, field) => getItemRendererComponent({ render, d, field, user, dispatch });
  ...
  const renderCellContent = (tableHeader, d) => {
    ...
    const value = getObjectValue(d, field);
  ...
  const renderChildCheckBox = (d) => {
    ...
    let hasOwnDataInSelection = null;
    if (selectedData) {
      hasOwnDataInSelection = Object.keys(selectedData).filter((key) => hasOwnRow(key, d, primaryKey));
      if (hasOwnDataInSelection && hasOwnDataInSelection.length > 0) {
        showSelected = true;
      }
    }
  ...
  if (showChild) {
    cData = resources.map((resource, resourceIndex) => (
      <Tr key={`${resource.id}-${resourceIndex + 1}`} className="child-row">
        {renderChildCheckBox(resource)}
        {columns.map((tableHeader, columnIndex) => (
          <td
            key={`${resource.id}-${tableHeader.field}-${columnIndex + 1}`}
