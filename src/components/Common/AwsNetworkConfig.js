...
class AwsNetworkConfig extends Component {
  componentDidMount() {
  ...
  renderRows() {
    ...
    return getNetworks.map((nic, index) => {
      ...
      const title = `Public IP : ${info.hasPublicIP}, Subnet: ${info.subnet}, Private IP: ${info.privateIP}`;
      return (
        <tr key={key}>
          <td>
            <Label>
...
export default AwsNetworkConfig;
