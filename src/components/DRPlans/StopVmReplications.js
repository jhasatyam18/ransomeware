...
const StopVmReplications = (props) => {
  const { drPlans, t, dispatch, location } = props;
  ...
  useEffect(() => {
    ...
    dispatch(fetchDRPlanById(parts[parts.length - 2], workflow));
  }, [refresh]);
  ...
  return (
    <Card>
      <CardBody>
        ...
        <SelectiveReplicationVm protectionPlan={protectionPlan} dispatch={dispatch} t={t} location={location} />
      </CardBody>
    </Card>
  );
};
