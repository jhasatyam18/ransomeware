...
function SelectiveReplicationVm(props) {
  ...
  const { dispatch, user, t, protectionPlan, history } = props;
  const { values } = user;
  ...
  useEffect(() => {
    ...
    dispatch(valueChange('ui.stop.repl.type', stopReplType));
...
export default connect(mapStateToProps)(withTranslation()(withRouter(SelectiveReplicationVm)));
