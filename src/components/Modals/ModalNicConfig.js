...
class ModalNicConfig extends Component {
  ...
  renderAWSConfig() {
    ...
    const securityGroup = { label: 'Security  Groups', placeHolderText: 'Security group', description: '', type: FIELD_TYPE.SELECT_SEARCH, shouldShow: true, validate: (v, u) => isEmpty(v, u), errorMessage: 'Select security group', COMPONENT: MULTISELECT_ITEM_COMP, options: (u, k) => getSecurityGroupOption(u, k), fieldInfo: 'info.protectionplan.network.aws.security.group' };
