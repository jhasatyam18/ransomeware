...
class ModalNicConfig extends Component {
  ...
  showWarningMessage() {
    ...
    const flow = (workflow !== UI_WORKFLOW.EDIT_PLAN && workflow !== UI_WORKFLOW.REVERSE_PLAN);
  ...
  renderAWSConfig() {
    ...
    const network = { fieldInfo: 'info.protectionplan.network.aws.elasticip', label: 'Elastic IP for instance', placeHolderText: 'Elastic IP', description: '', type: FIELD_TYPE.SELECT, shouldShow: true, errorMessage: 'Select external', options: (u, f) => getAWSElasticIPOptions(u, f) };
  ...
  renderAzureConfig() {
    ...
    const securityGroup = { label: 'Security  Groups', placeHolderText: 'Security group', description: '', type: FIELD_TYPE.SELECT_SEARCH, shouldShow: true, options: (u, k) => getAzureSecurityGroupOption(u, k), fieldInfo: 'info.protectionplan.network.azure.security.grp' };
    const privateIPField = { fieldInfo: 'info.protectionplan.network.gcp.privateip', label: 'Private IP', placeHolderText: 'Assign New', description: '', type: FIELD_TYPE.TEXT, shouldShow: true, validate: (v, u) => validateOptionalIPAddress(v, u), errorMessage: 'Invalid ip address or ip is not in subnet cidr range' };
          ...
          <Card>
            <CardBody>
              ...
              <Form>
                ...
                <DMFieldText dispatch={dispatch} fieldKey={`${networkKey}-privateIP`} field={privateIPField} user={user} />
                <DMFieldSelect dispatch={dispatch} fieldKey={`${networkKey}-publicIP`} field={publicIP} user={user} />
                <DMSearchSelect dispatch={dispatch} fieldKey={`${networkKey}-securityGroups`} field={securityGroup} user={user} />
              </Form>
            </CardBody>
          </Card>
          <div className="modal-footer">
