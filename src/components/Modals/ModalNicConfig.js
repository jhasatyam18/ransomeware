...
class ModalNicConfig extends Component {
  ...
  renderAzureConfig() {
    ...
    const { networkKey } = options;
    const networkField = { fieldInfo: 'info.protectionplan.network.azure.network', label: 'Network', description: '', type: FIELD_TYPE.SELECT, options: (u, f) => getAzureNetworkOptions(u, f), validate: (value, u) => isEmpty(value, u), errorMessage: 'Select network', shouldShow: true };
    ...
    const publicIP = { fieldInfo: 'info.protectionplan.network.azure.externalip', label: 'Public IP', placeHolderText: 'Assign New', description: '', type: FIELD_TYPE.SELECT, shouldShow: true, errorMessage: 'Select external', options: (u) => getAzureExternalIPOptions(u), validate: (v, u) => isEmpty(v, u) };
    const securityGroup = { label: 'Security  Groups', placeHolderText: 'Security group', description: '', type: FIELD_TYPE.SELECT, shouldShow: true, validate: (v, u) => isEmpty(v, u), errorMessage: 'Select security group', options: (u, k) => getAzureSecurityGroupOption(u, k), fieldInfo: 'info.protectionplan.network.azure.security.grp' };
    const privateIPField = { fieldInfo: 'info.protectionplan.network.gcp.privateip', label: 'Private IP', placeHolderText: 'Assign New', description: '', type: FIELD_TYPE.TEXT, shouldShow: true, validate: (v, u) => validateOptionalIPAddress(v, u), errorMessage: 'Invalid ip address or ip is not in subnet cidr range' };
