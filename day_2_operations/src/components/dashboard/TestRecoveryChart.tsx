...
interface ReplicationSeriesProps {
    ...
    rto?: number[]; // assuming rto is optional in Chart
}
interface PageInfo {
    currentPage: number;
    totalPages: number;
...
const BarRecoveeryStat: React.FC<Props> = ({ dispatch, user }) => {
    ...
    const radioSelection = getValue({ key: 'radio-button-testRecoveryStat', values }) || 'workloadView';
    ...
    const fetchData = async (offset = 0, siteIDs?: string[], durationValue?: string) => {
        ...
        let url = `${API_DASHBOARD_RECOVERY_BAR_STATS}limit=${count}&recoveryType=test recovery&groupBy=${groupBy}&viewType=${viewType}${siteParam}`;
        if (offset > 0) {
            url += `&offset=${offset}`;
        }
        const effectiveDuration = durationValue || 'month';
        const { start, end } = getStartEndTime(effectiveDuration, user, 'test.recovery.time.duration');
        ...
        callAPI(url).then(
            (json) => {
                ...
                let { records } = json;
                const seriesData = structureDataForChart(records, 'radio-button-testRecoveryStat', user, 'test-recovery');
    ...
    const renderHoverInfo = () => {
        return (
            <>
                <div>
                    <span style={{ fontWeight: '500', fontSize: '11px' }}>Shows test recovery status across your top target sites/plans.</span>
                    <ul className="mt-2">
                        <li>Target Sites/Plans: Only recoveries from these are included.</li>
                        <li>Passed: Completed successfully within expected RTO.</li>
                        <li>Failed: Did not complete or had errors.</li>
                        <li>Not Tested: Recovery tests not executed recently.</li>
    ...
    return (
        <Card>
            <CardBody className="box-shadow" style={{ minHeight: '315px' }}>
                <CardHeaderWithInfo text={`Test Recoveries By Site (Top ${count})`} IconText={renderHoverInfo} />
                {loader ? (
                    <>
                    ...
                    <div className="d-container">
                        <div className="d-child2">
                                ...
                                <div className={`d-flex align-items-center ${lavel === '1' ? 'graph_actions_resp_width' : 'w-45 justify-content-end '}`} style={{ gap: '10px' }}>
                                    ...
                                    {lavel === '1' ? (
                                        <div className="dash_radio_button_width" style={{ minHeight: '20px', textAlign: 'right' }}>
                                            <RadioOption fieldkey="radio-button-testRecoveryStat" />
                                        </div>
                                    ) : null}
                                    {lavel === '1' ? (
                                        <div className="dash_site_filter_width" style={{ minHeight: '10px', textAlign: 'right', marginRight: '5px' }}>
                                            <SiteSelectionDropdown user={user} fieldkey="test.recovery.site.dropdown" options={filteredOptions} icon={faFilter} checkbox onApply={handleSiteFilterApply} />
                                        </div>
                                    ) : null}
                                    ...
                                    <div className="dash_site_duration_width" style={{ minHeight: '10px', marginRight: '10px' }}>
                                        <DurationDropdown
                                            user={user}
                                            fieldkey="test.recovery.time.duration"
                            ...
                            {chartData.series && chartData.series.length > 0 && <StackedBarChart column={RECOVERY_JOBS} seriesData={chartData} showLabel={true} colors={[SUCCESS, WARNING, '#d88080']} height={240} onBarClicked={lavel === '1' ? onReplicationBarChartClick : null} onNavigateToReplication={lavel !== '1' ? '/dop/recovery' : null} fieldkey="testRecovery" />}
                            <div className="text-center dashboard_rec_heading_size">
                                <div>
                                    <span>{`${chartData.series.length > 0 ? `(Data For ${getDurationText(duration, user, 'test.recovery.time.duration')})` : 'No Test Recovery Data Available'}`}</span>
                                </div>
                            </div>
                        </div>
                    </div>
